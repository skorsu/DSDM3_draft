...
foreach(t = 1:20) %dopar% {
  ...
  ### ZIDM-sDM
  start_time <- Sys.time()
  mod <- ZIDM_DM(iter = 5000, Kmax = 10, z = as.matrix(dat[[t]]$dat),
                 atrisk_init = matrix(1, nrow = 150, ncol = 150),
                 beta_init = matrix(0, nrow = 10, ncol = 150), 
                 ci_init = rep(0, 150), theta = 1e-2, mu = 0, s2 = 0.1, 
                 s2_MH = 1e-3, t_thres = 1000, launch_iter = 30, 
                 r0g = 1, r1g = 1, thin = 1)
  tot_time <- difftime(Sys.time(), start_time, units = "secs")
  saveRDS(list(mod = mod, time = tot_time), 
          file = paste0(path, "Result/Simulation Study/simu_data_case_IV_chain_", t, "_ZsD.rds"))
