...
foreach(t = 1:20) %dopar% {
  ...
  ### ZIDM-ZIDM
  start_time <- Sys.time()
  mod <- mod_adaptive(iter = 5000, Kmax = 10, nbeta_split = 20, 
  ...
  saveRDS(list(mod = mod, time = tot_time), 
          file = paste0(path, "Result/Simulation Study/simu_data_case_III_chain_", t, "_ZZ.rds"))
  
  ### DM-ZIDM
  start_time <- Sys.time()
  mod <- DMZIDM(iter = 5000, Kmax = 10, z = as.matrix(dat[[t]]$dat), 
                beta_mat = matrix(0, nrow = 10, ncol = 100), 
                ci_init = rep(0, 50), theta = 1, launch_iter = 30, 
                r0c = 1, r1c = 4, thin = 1)
  tot_time <- difftime(Sys.time(), start_time, units = "secs")
  saveRDS(list(mod = mod, time = tot_time), 
          file = paste0(path, "Result/Simulation Study/simu_data_case_III_chain_", t, "_DZ.rds"))
...
resultDTMM <- foreach(t = 1:20) %dopar% {
  ...
  clus_result <- adjDTMM(dat[[t]]$dat, 
                         tree = read.tree(text = paste("(", paste(1:100, collapse = ", "), ");")), 
                         init_c = rep(1, 50), mcmc_iter = 5000)
  tot_time <- difftime(Sys.time(), start_time, units = "secs")
  list(time = tot_time, result = t(clus_result$post_c))
  saveRDS(list(mod = mod, time = tot_time), 
          file = paste0(path, "Result/Simulation Study/simu_data_case_III_chain_", t, "_DMDP.rds"))
