...
### Import the data
dat_case <- readRDS(paste0(path, "Data/Simulation Study/simu_data_case_V.rds"))
dat <- dat_case[[1]]$dat ### Data Used

### Hyperparameters
hyperSet <- matrix(NA, ncol = 7, nrow = 15)
colnames(hyperSet) <- c("Kmax", "n_eta", "theta", "s2", "s2MH", "bg", "bc")
hyperSet[1, ] <- c(10, 25, 1, 0.1, 1e-3, 1, 9) ### Default

### Kmax
hyperSet[2, ] <- c(5, 25, 1, 0.1, 1e-3, 1, 9)
hyperSet[3, ] <- c(25, 25, 1, 0.1, 1e-3, 1, 9)

### n_eta
hyperSet[4, ] <- c(10, 10, 1, 0.1, 1e-3, 1, 9)
hyperSet[5, ] <- c(10, 50, 1, 0.1, 1e-3, 1, 9)

### theta
hyperSet[6, ] <- c(10, 25, 0.1, 0.1, 1e-3, 1, 9)
hyperSet[7, ] <- c(10, 25, 10, 0.1, 1e-3, 1, 9)

### s2
hyperSet[8, ] <- c(10, 25, 1, 0.01, 1e-3, 1, 9)
hyperSet[9, ] <- c(10, 25, 1, 1, 1e-3, 1, 9)

### s2MH
hyperSet[10, ] <- c(10, 25, 1, 0.1, 1e-1, 1, 9)
hyperSet[11, ] <- c(10, 25, 1, 0.1, 1e-5, 1, 9)

### bg
hyperSet[12, ] <- c(10, 25, 1, 0.1, 1e-3, 4, 9)
hyperSet[13, ] <- c(10, 25, 1, 0.1, 1e-3, 9, 9)

### bc
hyperSet[14, ] <- c(10, 25, 1, 0.1, 1e-3, 1, 1)
hyperSet[15, ] <- c(10, 25, 1, 0.1, 1e-3, 1, 4)
...
foreach(t = 1:15) %dopar% {
  ...
  saveRDS(list(mod = mod, time = tot_time), 
          file = paste0(path, "Result/Sensitivity/sensitivity_scenario_V_case_", (t-1), "_.rds"))
