...
### Import the data
dat_case <- readRDS(paste0(path, "Data/Simulation Study/simu_data_case_IV.rds"))
...
foreach(t = 1:15) %dopar% {
  ...
  mod <- mod_adaptive(iter = 5000, Kmax = hyperSet[t, "Kmax"], nbeta_split = hyperSet[t, "n_eta"], 
                      z = as.matrix(dat), atrisk_init = matrix(1, nrow = 150, ncol = 150), 
                      beta_init = matrix(0, nrow = hyperSet[t, "Kmax"], ncol = 150), 
  ...
  saveRDS(list(mod = mod, time = tot_time), 
          file = paste0(path, "Result/Sensitivity/sensitivity_scenario_IV_case_", (t-1), "_.rds"))
