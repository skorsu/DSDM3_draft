...
# testResult2 <- readRDS(paste0(path, "Manuscript/Result/microbiome_result_6m_singleton.RData"))
# testResult3 <- readRDS(paste0(path, "Manuscript/Result/microbiome_result_6m_init30.RData"))
# testResult4 <- readRDS(paste0(path, "Manuscript/Result/microbiome_result_6m_init60.RData"))
...
#   facet_wrap(. ~ name) +
#   ylim(1, 10) + 
#   labs(title = TeX("Active Clusters: Adaptive Metropolis-Hasting for $\\beta$ (One Cluster)"))
# 
...
### Run with no adaptive part (want to show the problems): ---------------------
set.seed(1415, kind = "L'Ecuyer-CMRG")
start_ova <- Sys.time()
...
testResult1 <- foreach(t = 1:5) %dopar% {
  ...
  clus_result <- mod(iter = 15000, Kmax = 10, nbeta_split = 1,
                     ...
                     beta_init = matrix(0, ncol = 38, nrow = 10),
                     ci_init = rep(0, 90), theta = 1, mu = 0,
                     s2 = 1, s2_MH = 1e-3, launch_iter = 30, 
                     r0g = 4, r1g = 1, r0c = 1, r1c = 1,
                     thin = 1)
  tot_time <- difftime(Sys.time(), start_time, units = "secs")
  list(time = tot_time, result = clus_result)
...
stopImplicitCluster()
difftime(Sys.time(), start_ova)
saveRDS(testResult1, paste0(path, "Manuscript/Result/stat600proj_non_adaptive_1en3.RData"))
...
stopImplicitCluster()
difftime(Sys.time(), start_ova)
saveRDS(testResult1, paste0(path, "Manuscript/Result/stat600proj_adaptive_1en3.RData"))

saveRDS(dat06, paste0(path, "Manuscript/Result/dat06.RData"))
...
stopImplicitCluster()
difftime(Sys.time(), start_ova)
saveRDS(testResult1, paste0(path, "Manuscript/Result/microbiome_result_8m_oneClus.RData"))
rm(testResult1)
...
testResult2 <- foreach(t = 1:5) %dopar% {
  ...
  clus_result <- mod_adaptive(iter = 15000, Kmax = 90, nbeta_split = 1,
                              z = as.matrix(dat08[, -c(1:5)]), atrisk_init = matrix(1, ncol = 38, nrow = 90),
                              beta_init = as.matrix(dat08[, -c(1:5)])/rowSums(as.matrix(dat08[, -c(1:5)])),
...
stopImplicitCluster()
difftime(Sys.time(), start_ova)
saveRDS(testResult2, paste0(path, "Manuscript/Result/microbiome_result_8m_singleton.RData"))
rm(testResult2)
...
testResult3 <- foreach(t = 1:5) %dopar% {
  ...
  for(i in 1:30){
    beta_mat_init[i, ] <- as.numeric(colSums(dat08[which(ci_init_30 == i), -(1:5)])/sum(dat08[which(ci_init_30 == i), -(1:5)]))
  }
...
stopImplicitCluster()
difftime(Sys.time(), start_ova)
saveRDS(testResult3, paste0(path, "Manuscript/Result/microbiome_result_8m_init30.RData"))
rm(testResult3)
...
testResult4 <- foreach(t = 1:5) %dopar% {
  ...
  for(i in 1:60){
    beta_mat_init[i, ] <- as.numeric(colSums(dat08[which(ci_init_60 == i), -(1:5)])/sum(dat08[which(ci_init_60 == i), -(1:5)]))
  }
...
stopImplicitCluster()
difftime(Sys.time(), start_ova)
saveRDS(testResult4, paste0(path, "Manuscript/Result/microbiome_result_8m_init60.RData"))
rm(testResult4)
