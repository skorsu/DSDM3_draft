...
resultpath <- paste0(path, "Result/david/")
# file.exists(resultpath)

## Metadata
metadata <- read.delim(paste0(datapath, "david/task.txt"))
# metadata_mapping <- read.delim(paste0(datapath, "david/mapping-orig.txt"))
View(metadata)
...
taxaSplit <- do.call(rbind.data.frame, 
                     lapply(dat$X.OTU.ID, function(x){str_replace(str_split_fixed(x, "\\;", 7), "\\([:digit:]+\\)\\;*", "")}))
taxaSplit <- taxaSplit[, 1:6]
colnames(taxaSplit) <- c("k", "p", "c", "o", "f", "g")
distinctTaxa <- taxaSplit %>% distinct()
dat <- dat %>% dplyr::select(-X.OTU.ID)
...
### Column that have the same taxonomy
start_cleaned <- Sys.time()
david_genus <- matrix(NA, nrow = 770, ncol = 459)
david_genusCOL <- rep(NA, 770)
j <- 1

for(i in 1:770){
  sameTaxa <- which(sapply(1:1535, function(x){sum(taxaSplit[x, ] == distinctTaxa[i, ])}) == 6)
  
  if(length(sameTaxa) != 0){
    if(length(sameTaxa) == 1){
      david_genus[j, ] <- as.numeric(dat[sameTaxa, ])
    } else {
      david_genus[j, ] <- as.numeric(colSums(dat[sameTaxa, ]))
    }
    david_genusCOL[j] <- paste(distinctTaxa[i, ], collapse = ";")
    j <- j + 1
  }
...
rownames(david_genus) <- david_genusCOL
colnames(david_genus) <- colnames(dat)
david_genus <- t(david_genus)
david_genus <- david_genus[, -which(colMeans(david_genus > 0) < 0.1)]
david_genus <- david_genus[which(rownames(david_genus) %in% metadata$X.SampleID), ]
dim(david_genus)

data.frame(ID = names(kmeans(david_genus, 2)$cluster),
           clus = kmeans(david_genus, 2)$cluster) %>%
  inner_join(metadata) %>%
  dplyr::select(-ID) %>%
  table()

dim(david_genus)
saveRDS(david_genus, paste0(datapath, "david/clean_david.rds"))

data.frame(ID = 1:170, p = sort(colSums(david_genus)/sum(david_genus), decreasing = TRUE)) %>%
  ggplot(aes(x = ID, y = p)) +
  geom_bar(stat = "identity")

x <- rnorm(170, 0, sqrt(5))
data.frame(x = 1:170, p = sort(exp(x)/sum(exp(x)), decreasing = TRUE)) %>%
  ggplot(aes(x = x, y = p)) +
  geom_bar(stat = "identity")

mod <- mod_adaptive(iter = 3000, Kmax = 20, nbeta_split = 1,
                    z = as.matrix(david_genus), atrisk_init = matrix(1, nrow = 19, ncol = 170),
                    beta_init = matrix(0, nrow = 20, ncol = 170),
                    ci_init = rep(0, 19),
                    theta = 1, mu = 0, s2 = 5, s2_MH = 1e-5,
...
data.frame(ID = rownames(david_genus),
           clus = as.numeric(salso(mod$ci_result[-(1:1000), ]))) %>%
  inner_join(metadata) %>%
  dplyr::select(-ID) %>%
  table()

### Read the data
otuTab <- readRDS(paste0(datapath, "david/clean_david.rds"))
...
set.seed(1)
ciInit <- matrix(0, nrow = 19, ncol = 12)
ciInit[, 4] <- sample(0:2, 19, replace = TRUE)
ciInit[, 5] <- sample(0:2, 19, replace = TRUE)
ciInit[, 6] <- sample(0:2, 19, replace = TRUE)
ciInit[, 7] <- sample(0:4, 19, replace = TRUE)
ciInit[, 8] <- sample(0:4, 19, replace = TRUE)
ciInit[, 9] <- sample(0:4, 19, replace = TRUE)
ciInit[, 10] <- sample(0:9, 19, replace = TRUE)
ciInit[, 11] <- sample(0:9, 19, replace = TRUE)
ciInit[, 12] <- sample(0:9, 19, replace = TRUE)
...
xiInit[[1]] <- matrix(0, nrow = 15, ncol = 170)
xiInit[[2]] <- matrix(0, nrow = 15, ncol = 170)
xiInit[[3]] <- matrix(0, nrow = 15, ncol = 170)

xiInit[[4]] <- rbind(xiInitDum[[1]], matrix(0, nrow = 12, ncol = 170))
xiInit[[5]] <- rbind(xiInitDum[[2]], matrix(0, nrow = 12, ncol = 170))
xiInit[[6]] <- rbind(xiInitDum[[3]], matrix(0, nrow = 12, ncol = 170))

xiInit[[7]] <- rbind(xiInitDum[[4]], matrix(0, nrow = 10, ncol = 170))
xiInit[[8]] <- rbind(xiInitDum[[5]], matrix(0, nrow = 10, ncol = 170))
xiInit[[9]] <- rbind(xiInitDum[[6]], matrix(0, nrow = 10, ncol = 170))

xiInit[[10]] <- xiInitDum[[7]]
xiInit[[11]] <- xiInitDum[[8]]
xiInit[[12]] <- xiInitDum[[9]]

resultName <- c(paste0("result_david_chain_", 1:3, "_init_oneClus_s2_1en1_s2MH_1en3.rds"),
                paste0("result_david_chain_", 1:3, "_init_3clus_s2_1en1_s2MH_1en3.rds"),
                paste0("result_david_chain_", 1:3, "_init_5clus_s2_1en1_s2MH_1en3.rds"),
                paste0("result_david_chain_", 1:3, "_init_10clus_s2_1en1_s2MH_1en3.rds"))
...
foreach(t = 1:6) %dopar% {
  start_time <- Sys.time()
  mod <- mod_adaptive(iter = 25000, Kmax = 15, nbeta_split = 1,
                      z = as.matrix(otuTab), atrisk_init = matrix(1, nrow = 19, ncol = 170),
